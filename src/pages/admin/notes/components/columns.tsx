...
import { NoteResponseDto, NoteStatus } from "@/types/note";
...
export const columns: ColumnDef<NoteResponseDto>[] = [
    {
        accessorKey: "folio",
        header: "Folio",
    ...
    {
        accessorKey: "weakClientName",
        header: "Cliente",
        cell: ({ row }) => {
            return (
                <div className="flex flex-col">
                    <span className="font-medium text-gray-900">{row.getValue("weakClientName")}</span>
                </div>
            );
        }
    },
    {
        ...
        cell: ({ row }) => {
            return new Date(row.getValue("date")).toLocaleDateString("es-MX");
        },
    },
    {
        accessorKey: "dueDate",
